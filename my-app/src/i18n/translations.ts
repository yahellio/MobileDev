import type { Language } from '../types/app';

export const translations = {
  ru: {
    splashTitle: 'Giga Tracker',
    splashSubtitle: 'Загрузка приложения...',
    appTitle: 'История тренировок',
    settings: 'Настройки',
    details: 'Детали тренировки',
    addWorkout: 'Добавить',
    edit: 'Редактировать',
    delete: 'Удалить',
    save: 'Сохранить',
    cancel: 'Отмена',
    noWorkouts: 'Пока нет тренировок. Добавьте первую.',
    duration: 'Время',
    date: 'Дата',
    minutesShort: 'мин',
    exercises: 'Упражнения',
    noExercises: 'Упражнения не добавлены',
    selectExercises: 'Выбрать упражнения',
    chooseExercises: 'Выбрать',
    selectedExercises: 'Выбранные упражнения',
    noSelectedExercises: 'Ничего не выбрано',
    muscleGroups: 'Группы мышц',
    title: 'Название',
    description: 'Описание',
    exerciseList: 'Список упражнений',
    durationMinutes: 'Длительность (мин)',
    createWorkout: 'Создание тренировки',
    updateWorkout: 'Редактирование тренировки',
    userName: 'Имя пользователя',
    language: 'Язык',
    darkTheme: 'Темная тема',
    lightTheme: 'Светлая тема',
    greeting: 'Привет',
    back: 'Назад',
    quoteLoading: 'Загрузка цитаты...',
    quoteUnavailable: 'Цитата недоступна',
    quoteShakeHint: 'Встряхните телефон — новая цитата',
    onlineStatus: 'Онлайн',
    offlineStatus: 'Оффлайн',
    catalogLoading: 'Загрузка упражнений...',
    catalogUnavailable: 'Каталог упражнений недоступен',
    searchPlaceholder: 'Поиск...',
    noSearchResults: 'Ничего не найдено',
    sortNewestFirst: 'Сначала новые',
    sortOldestFirst: 'Сначала старые',
    sortByTitle: 'По названию',
    remindersDaily: 'Напоминания о тренировке',
    remindersDailyHint: 'Каждый день в 10:00',
    remindersTestButton: 'Тест: уведомление сейчас',
    remindersPermissionDenied: 'Включите уведомления для приложения в настройках системы.',
    remindersWebUnavailable: 'Уведомления доступны в приложении на телефоне, не в веб-версии.',
    workoutPhoto: 'Фото тренировки',
    pickWorkoutPhoto: 'Выбрать фото',
    clearWorkoutPhoto: 'Убрать фото',
    authTitle: 'Вход',
    authEmail: 'Email',
    authPassword: 'Пароль',
    authDisplayName: 'Имя',
    authDisplayNamePlaceholder: 'Как к вам обращаться',
    authSignIn: 'Войти',
    authSignUp: 'Регистрация',
    authModeSignIn: 'Вход',
    authModeRegister: 'Регистрация',
    authHintSignIn: 'Войдите в аккаунт',
    authHintRegister: 'Создайте аккаунт',
    authSignOut: 'Выйти',
    accountEmail: 'Аккаунт',
    authFirebaseRequired: 'Firebase не настроен. Проверьте переменные окружения.',
    authErrorGeneric: 'Не удалось выполнить операцию. Попробуйте снова.',
    authErrorInvalid: 'Неверный email или пароль.',
    authErrorEmailInUse: 'Этот email уже зарегистрирован.',
    authErrorWeakPassword: 'Пароль слишком простой.',
    authErrorInvalidEmail: 'Некорректный email.',
  },
  en: {
    splashTitle: 'Giga Tracker',
    splashSubtitle: 'Loading app...',
    appTitle: 'Workout History',
    settings: 'Settings',
    details: 'Workout Details',
    addWorkout: 'Add',
    edit: 'Edit',
    delete: 'Delete',
    save: 'Save',
    cancel: 'Cancel',
    noWorkouts: 'No workouts yet. Add your first one.',
    duration: 'Duration',
    date: 'Date',
    minutesShort: 'min',
    exercises: 'Exercises',
    noExercises: 'No exercises added',
    selectExercises: 'Select exercises',
    chooseExercises: 'Choose',
    selectedExercises: 'Selected exercises',
    noSelectedExercises: 'Nothing selected',
    muscleGroups: 'Muscle groups',
    title: 'Title',
    description: 'Description',
    exerciseList: 'Exercise list',
    durationMinutes: 'Duration (minutes)',
    createWorkout: 'Create workout',
    updateWorkout: 'Update workout',
    userName: 'User name',
    language: 'Language',
    darkTheme: 'Dark theme',
    lightTheme: 'Light theme',
    greeting: 'Hi',
    back: 'Back',
    quoteLoading: 'Loading quote...',
    quoteUnavailable: 'Quote is unavailable',
    quoteShakeHint: 'Shake the device for a new quote',
    onlineStatus: 'Online',
    offlineStatus: 'Offline',
    catalogLoading: 'Loading exercises...',
    catalogUnavailable: 'Exercise catalog is unavailable',
    searchPlaceholder: 'Search...',
    noSearchResults: 'No matches',
    sortNewestFirst: 'Newest first',
    sortOldestFirst: 'Oldest first',
    sortByTitle: 'By title',
    remindersDaily: 'Workout reminders',
    remindersDailyHint: 'Every day at 10:00',
    remindersTestButton: 'Test: notify now',
    remindersPermissionDenied: 'Enable notifications for this app in system settings.',
    remindersWebUnavailable: 'Notifications work in the mobile app, not on web.',
    workoutPhoto: 'Workout photo',
    pickWorkoutPhoto: 'Choose photo',
    clearWorkoutPhoto: 'Remove photo',
    authTitle: 'Sign in',
    authEmail: 'Email',
    authPassword: 'Password',
    authDisplayName: 'Name',
    authDisplayNamePlaceholder: 'How should we call you',
    authSignIn: 'Sign in',
    authSignUp: 'Register',
    authModeSignIn: 'Sign in',
    authModeRegister: 'Register',
    authHintSignIn: 'Sign in to your account',
    authHintRegister: 'Create an account',
    authSignOut: 'Sign out',
    accountEmail: 'Account',
    authFirebaseRequired: 'Firebase is not configured. Check environment variables.',
    authErrorGeneric: 'Something went wrong. Please try again.',
    authErrorInvalid: 'Invalid email or password.',
    authErrorEmailInUse: 'This email is already registered.',
    authErrorWeakPassword: 'Password is too weak.',
    authErrorInvalidEmail: 'Invalid email address.',
  },
} as const;

export type TranslationDictionary = (typeof translations)[Language];
